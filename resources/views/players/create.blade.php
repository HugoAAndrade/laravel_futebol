...
<form action="{{ route('players.store') }}" method="POST">
    ...
    <div class="mb-4">
        ...
        <input type="text" name="name" required class="w-full p-2 rounded bg-slate-800 text-white border-none focus:outline-none">
    </div>
    <div class="mb-4">
        <label for="level" class="block text-lg font-bold mb-2">Nível</label>
        <input type="number" name="level" min="1" max="5" required
            class="w-full p-2 rounded bg-slate-800 text-white border-none focus:outline-none">
    </div>
    <div class="mb-4">
        <label for="is_goalkeeper" class="block text-lg font-bold mb-2">É goleiro?</label>
        <select name="is_goalkeeper" required class="w-full p-2 rounded bg-slate-800 text-white border-none focus:outline-none">
